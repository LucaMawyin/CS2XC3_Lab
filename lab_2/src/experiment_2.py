...
for i,ea in enumerate(edge_amounts):
    ...
    for _ in range(TEST_AMOUNT):
        ...
        probability += 1 if is_connected(graph) else 0
...
plt.title(f'Probabilties graphs being connected with {NODE_AMOUNT} nodes')
plt.xlabel('Number of edges')
plt.ylabel('% chance the graph is connected')
plt.grid(True)
