...
numberOfGraphs = 1000
maxNumberOfEdgesScale = 0.5
nodeSizeList = [n for n in range(1,10)]
results = {}

for nodeSize in nodeSizeList:
    maxEdges = nodeSize * (nodeSize - 1) // 2
    numberOfEdges = int(maxNumberOfEdgesScale * maxEdges)
    ...
    for _ in range(numberOfGraphs):
        G = create_random_graph2(nodeSize, numberOfEdges)
        mvcSum += len(MVC(G))
